...
EDIN Relational Schema:
Call `galaxy_schema` before using `galaxy_query`; the live schema output is authoritative. Current state lives in `galaxy.*`. Join tables through identifiers such as `system_id64`, `market_id`, `body_id`, and `faction_id`. Coordinates in `galaxy.system_catalog` are stored in light years.

**Query Rules:**
- Read-only `SELECT` or `WITH` statements only.
- Always include a bounded `LIMIT`.
