...
func MCPToolDefinitions() []mcp.Tool {
	return []mcp.Tool{
		...
		mcp.NewTool(string(ToolGalaxySurfaceSites),
			mcp.WithDescription("Find notable landable-surface sites near a reference system from the relational galaxy database. Use for Ancient Ruins, Biological Sites, geysers/fumaroles, visitor beacons, crash sites, and similar reported sites."),
			mcp.WithString("system_name", mcp.Required(), mcp.Description("Reference system name")),
			mcp.WithNumber("radius", mcp.Description("Search radius in Ly (default 100, max 500)")),
			mcp.WithString("site_kind", mcp.Description("Optional kind filter, e.g. Ancient, Biological, Geological, VisitorBeacon, CrashSite")),
			mcp.WithString("name", mcp.Description("Optional site-name filter, e.g. Ancient Ruins, Biological Site, Ice Geysers")),
		...
		mcp.NewTool(string(ToolGalaxyStats),
			mcp.WithDescription("Get relational galaxy database statistics"),
		),
		mcp.NewTool(string(ToolGalaxyQuery),
			mcp.WithDescription("Execute one ad-hoc read-only PostgreSQL SELECT/WITH query against the relational galaxy database. Parser-enforced sandbox; DDL/DML/session/transaction/control statements are rejected and results are capped."),
			mcp.WithString("query", mcp.Required(), mcp.Description("PostgreSQL SELECT/WITH query to execute. Use $1, $2, ... for parameters.")),
		...
		mcp.NewTool(string(ToolGalaxySchema),
			mcp.WithDescription("Return the current relational galaxy schema inventory: table columns and indexes. Use before writing ad-hoc SQL queries to understand available data structures."),
		),
...
var slimDescriptions = map[ToolName]string{
	...
	ToolGalaxySurfaceSites:      "Find surface sites by radius/kind/name. Use describe_tool for parameters.",
